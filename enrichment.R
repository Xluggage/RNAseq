...
if (grepl("cell", sample,ignore.case = TRUE)) {
  ...
  diffGene <- read.csv(
    paste0(
      ...
      "/Differential/",
      sample,
      "_DEG_logFC1.0_ihwP_filtered.csv"
    ),
    header = TRUE,
    check.names = FALSE
  )
...
symbol_up <-
  diffGene[diffGene$log2FoldChange > 0, "Symbol"] %>% na.omit()
symbol_down <-
  diffGene[diffGene$log2FoldChange < 0, "Symbol"] %>% na.omit()
...
ensembl_up <-
  diffGene[diffGene$log2FoldChange > 0, 1]
ensembl_down <-
  diffGene[diffGene$log2FoldChange < 0, 1]
...
entrez_up <-
  diffGene[diffGene$log2FoldChange > 0, "Entrez Gene ID"] %>% na.omit()
entrez_down <-
  diffGene[diffGene$log2FoldChange < 0, "Entrez Gene ID"] %>% na.omit()
...
allGene %<>% .[.$ihwP < 0.05, ]
# extract genes by adjust P value < 0.05
...
symbolListTmp <-
  allGene[, c("Symbol", "log2FoldChange")] %>% .[order(.$log2FoldChange, decreasing = TRUE), ] %>% na.omit()
symbolList_up <-
  symbolListTmp[symbolListTmp$log2FoldChange > 0, "Symbol"]
symbolList_down <-
  symbolListTmp[symbolListTmp$log2FoldChange < 0, "Symbol"]
...
rm(symbolListTmp)
# log2FoldChange value list, rownames is Symbol

ensemblListTmp <-
  allGene[, c("Gene ID", "log2FoldChange")] %>% .[order(.$log2FoldChange, decreasing = TRUE), ] %>% na.omit()
ensemblList <- ensemblListTmp[, 2]
names(ensemblList) <- ensemblListTmp[, 1]
rm(ensemblListTmp)
# log2FoldChange value list, rownames is Gene ID

entrezListTmp <-
  allGene[, c("Entrez Gene ID", "log2FoldChange")] %>% .[order(.$log2FoldChange, decreasing = TRUE), ] %>% na.omit()
entrezList <-  entrezListTmp[, 2]
names(entrezList) <- entrezListTmp[, 1]
rm(entrezListTmp)
# log2FoldChange value list, rownames is Entrez ID
...
gseaGO <- function(x,
  ...
  # x:              the list of genes that is not filtered by difference value is in the form of a string of log2FoldChange value vectors, sorted in descending order, with the gene name (ensembl ID) as the name of each item, and the parameter keyType for other gene IDs
  # gene_up/down:   a up/down subset of the differential gene list (Symbol), which is used to generate the up/down gene name and number after the results
  # ont:            a subset of the GO database, divided into three types: "BP", "CC", "MF", or analyzed together using "ALL"
  # keyType:        the type of gene IDs entered, "ENSEMBL" is ensembl ID, "ENTREZID" is Entrez ID, parameters: columns(org.Mm.eg.db)
...
gseaKEGG <- function(x,
  ...
  # x:              the list of genes that is not filtered by difference value is in the form of a string of log2FoldChange value vectors, sorted in descending order, with the gene name (Entrez ID) as the name of each item, and the parameter keyType for other gene IDs
  # gene_up/down:   a up/down subset of the differential gene list (Symbol), which is used to generate the up/down gene name and number after the results
  # keyType:        the type of gene IDs, includes "kegg", "ncbi-geneid", "ncbi-proteinid" and "uniprot". the "kegg" is Entrez ID (eukaryotes) or Locus ID (prokaryotes)
  # pvalue:         p valve cut-off value
  # min/max:        gene set size limit to screen for analysis (screening out overly general gene sets)
...
gseaReactome <- function(x,
  ...
  # x:              the list of genes that is not filtered by difference value is in the form of a string of log2FoldChange value vectors, sorted in descending order, with the gene name (Entrez ID) as the name of each item
  # gene_up/down:   a up/down subset of the differential gene list (Symbol), which is used to generate the up/down gene name and number after the results
  # pvalue:         p valve cut-off value
  # min/max:        gene set size limit to screen for analysis (screening out overly general gene sets)
  # organism:       organism species information, mouse is "mouse"
...
pathPlot <- function(x,
  ...
  # diffGene:       the list of differential genes, with at least three column: "Symbol", "Entrez Gene ID" and "log2FoldChange"
  ...
  geneDataList <-
    purrr::map(
      geneIDlist,
      ~ filter(diffGene, Symbol %in% .) %>% .[, c("Entrez Gene ID", "log2FoldChange")] %>% remove_rownames(.) %>% column_to_rownames(., var =
                                                                                                                                       "Entrez Gene ID")
    )
...
for (i in c("all", "up", "down")) {
  ...
  if (!is.null(get(paste0("KEGG_", i)))) {
    diffGene_notNA <- switch(
      i,
      all = drop_na(diffGene, Symbol, `Entrez Gene ID`, log2FoldChange),
      up = drop_na(diffGene[diffGene$log2FoldChange > 0,], Symbol, `Entrez Gene ID`, log2FoldChange),
      down = drop_na(diffGene[diffGene$log2FoldChange < 0,], Symbol, `Entrez Gene ID`, log2FoldChange)
    )
...
if (!is.null(KEGG_gsea)) {
  diffGene_notNA <-
    drop_na(allGene, Symbol, `Entrez Gene ID`, log2FoldChange)
  # extract data to add color for KEGG pathway plot
